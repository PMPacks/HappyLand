...
class FPerms extends Command implements PluginIdentifiableCommand
{
    ...
    */
    ...
    public function __construct(PureChich $plugin, $name, $description)
    {
        ...
        $this->setPermission("pchich.command.fperms");
    }
    ...
    public function execute(CommandSender $sender, string $label, array $args) : bool
    {
        ...
        {
            $sender->sendMessage(TextFormat::GREEN . PureChich::MAIN_PREFIX . ' ' . $this->plugin->getMessage("cmds.fperms.usage"));
        ...
        if($plugin === null)
        {
            $sender->sendMessage(TextFormat::RED . PureChich::MAIN_PREFIX . ' ' . $this->plugin->getMessage("cmds.fperms.messages.plugin_not_exist", $args[0]));
        ...
        {
            $sender->sendMessage(TextFormat::GREEN . PureChich::MAIN_PREFIX . ' ' . $this->plugin->getMessage("cmds.fperms.messages.no_plugin_perms", $plugin->getName()));
        ...
        $sender->sendMessage(TextFormat::GREEN . PureChich::MAIN_PREFIX . ' ' . $this->plugin->getMessage("cmds.fperms.messages.plugin_perms_list", ($plugin instanceof PluginBase) ? $plugin->getName(): 'PocketMine-MP', $pageNumber, $maxPageNumber));
        
        foreach($chunkedPermissions[$pageNumber - 1] as $permission)
        {
            $sender->sendMessage(TextFormat::GREEN . PureChich::MAIN_PREFIX . ' - ' . $permission->getName());
        }
