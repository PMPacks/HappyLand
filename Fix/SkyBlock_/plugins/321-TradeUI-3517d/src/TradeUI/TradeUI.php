...
class TradeUI extends PluginBase{
	...
	public function setEconomyProvider(EconomyProvider $provider){
        ...
        public function getBuyOrSellForm(Player $player) {
                ...
                $form->setTitle(RandomUtils::colorMessage("§l§b[§a TRAO ĐỎI HÀNG HÓA §b]"));
                $content = "";
               // $content .= RandomUtils::colorMessage("&l&e===========================\n");
                $content .= RandomUtils::colorMessage("&l§b6Xin Chào, {$username}!\n\n");
                $content .= RandomUtils::colorMessage("&l§aNếu bạn muốn bán mặt hàng của bạn nhấn §6[ B Á N ]\n");
                $content .= RandomUtils::colorMessage("&l§a Nếu bạn muốn mua mặt hàng nào đó nhấn §6[ M U A ]\n\n");
                /*$content .= RandomUtils::colorMessage("&r&dIf you want to sell your own items\n");
                $content .= RandomUtils::colorMessage("&r&dclick 'SELL'!\n");
                $content .= RandomUtils::colorMessage("&l&e===========================\n");*/
                $form->setContent($content);
                $form->setButton(RandomUtils::colorMessage("§6[ M U A ]"));
                $form->setButton(RandomUtils::colorMessage("§6[ B Á N ]"));
                $form->send($player);
        }
        ...
        public function getChooseItemForm(Player $player) {
                ...
                $form->setTitle(RandomUtils::colorMessage("§l§b CHỌN VẬT PHẨM ĐỄ BÁN "));
                $this->savePlayerItemNames($player);
                $items = $player->getInventory()->getContents();
                foreach($items as $key => $item){
                        $form->setButton(RandomUtils::colorMessage("§l§c[ {$item->getName()} &e(&8x{$item->getCount()}&e) §c]"),
