...
class FactionListener implements Listener {
	...
	public function factionChat(PlayerChatEvent $PCE) {
		...
		if($this->plugin->motdWaiting($player)) {
			if(time() - $this->plugin->getMOTDTime($player) > 30) {
				$PCE->getPlayer()->sendMessage($this->plugin->formatMessage("§e»§b Timed out. Please use /g desc again."));
				...
				$PCE->setCancelled(true);
				$PCE->getPlayer()->sendMessage($this->plugin->formatMessage("§e»§b Successfully updated the faction description. Type /g info.", true));
			}
			return true;
		}
	...
	}*/ //Like I said, I don't want to claim
