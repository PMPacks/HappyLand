...
class TipSender extends Task{
	...
	public function onRun($currentTick){  //当前时间
		if ($currentTick % 600 == 0){ //60*5*20 = 600 (5 min)
			$this->plugin->save();
			$this->plugin->getLogger()->info(TextFormat::GRAY."Đã lưu tự động");
		}
		...
		/*	if (PocketVIP::getInstance()->isSVIP($player->getName())){
			...
			///////////////////////////
			$player->sendPopup( "§l§c[ §a⚒ Level:§e". $grade."§c ]\n"."§l§c[ §a⚒§b EXP: §e".$exp."§c/§e".$expM."§c ]");
 
//		$player->sendPopup("§l§a[§e $grade §c|§6 $title §c|§b EXP:§a ".$exp."/".$expM." §a]");
			//$player->sendPopup($str);
		//	$player->sendPopup("§eLv. $grade $title | §b经验值：$exp/$expM | §a钱包: $money 元 | §d所在世界：$world | §c坐标：($x:$y:$z) $bearing \n§5节操：$mi | §b权限：$qx | §2在线人数：$online | §4公会：$ghs | §f手持：$id: $ts | §bTime: ".date("H")." :".date("i")." :".date("s")." ");
		}
		/*
