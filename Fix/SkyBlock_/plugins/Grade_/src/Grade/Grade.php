...
class Grade extends PluginBase implements Listener{
	...
	public function getEXP($player){
		...
		if (isset($this->exp[$player]["exp"]) == false){
			...
			$expM = $this->getMaxEXP($name);
			$grade = $this->getGrade($name);
			$this->getLogger()->info("§l§a Không tìm thấy data §6".$name."§a tạo data mới...");
			ScoreFactory::setScoreLine($player, 10, "§l§a⚒ Level:§e". $grade);
			ScoreFactory::setScoreLine($player, 11, "§l§a⚒§b EXP: §e".$exp."§c/§e".$expM);*/
		}
	...
	public function updatePlayer($player2, $hide = false){ //升级	
		...
		if (!$hide){
			$this->getServer()->broadcastMessage("§l§c[§a AURAKINGDOM §c]"." §l§d[§b{$player2->getName()}§d]§6 đã tăng lên thành §e{$this->getGrade($player)} !!"); //倾斜.绿色
...
/*	public function player_setMaxHealth(Player $player){
	...
	public function convert(Player $player, $id, $count){
		...
		if ($count <= 0){
			$player->sendMessage(TextFormat::AQUA."§lSố lượng không được nhỏ hơn 0!!");
			return false;
		}
		...
		switch($id){
			...
			case 2:
				...
				$num = 100 *$count;
				if ($num > $MI){
					$player->sendMessage(TextFormat::AQUA."{$count} Xu {$num} Fuck, bạn chỉ có {$MI} MI");
					return false;
				}

				EconomyAPI::getInstance()->addMoney($player, $count);
				$player->sendMessage(TextFormat::AQUA."Mua Thành Cong!!");
			...
			case 3:
				...
				$num = 500*$count;
				if ($num > $MI){
					$player->sendMessage(TextFormat::AQUA."{$count} Sắt {$num}, Bạn chỉ có {$MI} MI");
					return false;
				}
			...
			case 4:
				...
				$num = 800*$count;
				if ($num > $MI){
					$player->sendMessage(TextFormat::AQUA."{$count} Kiem Cuong Block {$num} Bạn chỉ có {$MI} MI");
					return false;
				}
			...
			case 5:
				...
				$num = 10*$count;
				if ($num > $MI){
					$player->sendMessage(TextFormat::AQUA."{$count} Kinh Nghiệm {$num} Bạn chỉ có {$MI} MI");
					return false;
				}
			...
			default:
				$player->sendMessage(TextFormat::AQUA."ID mục không hợp lệ !! Vui lòng sử dụng ID sau.");
				$player->sendMessage(TextFormat::YELLOW.
					"1.Diamond [1000 MI/1]; ".
					"2.XU [ 1 MI / 1000] ".
					"3.Khối sắt (500 MI / khối); ".
					"4.Vàng [500 MI / 1]".
					"5. Exp [10 MI / 1]"
				);
			break;
		...
		if($inventory->canAddItem($item) == false){
			$player->sendMessage(TextFormat::RED."Không đổi được gói!Inventory đã đầy!");
			return false;
		}
	...
	public function onEnable(){
		...
		$this->getScheduler()->scheduleRepeatingTask(new TipSender($this), 15);
		...
		$this->disposeConfig();
		$this->save();
		$this->getLogger()->info("§l§c⚒§6 GRADE §aENABLE§c ⚒");
		...
		switch($command->getName()){
			case "exp":
				...
				if (isset($args[0]) == false){
					$sender->sendMessage(TextFormat::RED."Usage: /exp setgrade | setexp");
					return true;
				}
				...
				switch ($args[0]) {
					case "setgrade":
						if (isset($args[1]) == false or isset($args[2]) == false){
							$sender->sendMessage(TextFormat::RED."Usage: /exp setgrade <player> <grade>");
							return true;
						}

						$this->setGrade($args[1], (int)$args[2]);
						$sender->sendMessage(TextFormat::AQUA."Set Thành Cong");
					return true;
		
					case "setexp":
						if (isset($args[1]) == false or isset($args[2]) == false){
							$sender->sendMessage(TextFormat::RED."Usage: /exp setexp <Player> <exp>");
							return true;
						}

						$this->setEXP($args[1], (int)$args[2]);
						//$this->updatePlayer($args[1]);
						$sender->sendMessage(TextFormat::AQUA."§lSet Thành công");
					return true;

					case "setmi":
						if (isset($args[1]) == false or isset($args[2]) == false){
							$sender->sendMessage(TextFormat::RED."Usage: /exp setmi <Player> <Mi>");
							return true;
						}
			...
			case "兑换":
				if (isset($args[0]) == false or isset($args[1]) == false){
					$sender->sendMessage(TextFormat::RED."usage: /convert <Số mặt hàng> <số luợng>");
					return true;
				}

				if (!$sender instanceof Player){
					//控制台
					$sender->sendMessage(TextFormat::RED."You not a Player");
					return true;
				}
			...
			case "setfix":
				...
				if (isset($args[0]) == false or isset($args[1]) == false){
					$sender->sendMessage(TextFormat::RED."使用/setfix <player> <Name>");
					return true;
				}
				...
				if ($player instanceof Player){
					...
					//$player->setNameTag(TextFormat::AQUA."Lv.".$this->getGrade($player)." ".$this->getTitle($player).TextFormat::GOLD." ".$player->getDisplayName());
					$sender->sendMessage(TextFormat::AQUA."Đặt thành công !!");
				} else {
					$sender->sendMessage(TextFormat::RED."Player Khong Online");
				}
			return true;
	...
	public function onChat(PlayerChatEvent $event){ 
		...
		//$msg = TextFormat::GOLD."[{$level}]".TextFormat::AQUA."<$ghs>".TextFormat::GREEN."[Lv.{$grade} {$title}]".TextFormat::YELLOW." {$name}: ".TextFormat::WHITE.;
		//$this->getServer()->broadcastMessage($msg);
		$event->getPlayer()->setDisplayName("§a[§6LV: {$grade} §e|§6 {$title}§a]§f"." ".$player->getName());
	//$event->setFormat("§b[{$level}]§a [§6{$grade} §e|§6 {$title}§a]"." ".$player->getDisplayName()." → §f".$event->getMessage());
	}
	...
	public function onJoin(PlayerJoinEvent $event){
		...
		//$event->setJoinMessage(TextFormat::BOLD.TextFormat::YELLOW."Player: ".TextFormat::AQUA." Lv.".$this->getGrade($player)." ".$this->getTitle($player).TextFormat::GOLD." ".$player->getDisplayName().TextFormat::GREEN." Tham Gia");
	//	$this->player_setMaxHealth($event->getPlayer());
		$player->setNameTag(TextFormat::AQUA."§lLv.".$this->getGrade($player)." ".$this->getTitle($player).TextFormat::GOLD." ".$player->getDisplayName());
	...
	public function onKick(PlayerKickEvent $event){
		$player = $event->getPlayer();
		$event->setQuitMessage(TextFormat::AQUA." Lv.".$this->getGrade($player)." ".$this->getTitle($player).TextFormat::GOLD." ".$player->getDisplayName().TextFormat::YELLOW." Bị quản trị viên loại khỏi trò chơi");
	}
