...
class Main extends PluginBase{
	...
	public function onEnable(): void{
		...
		//	$this->grade =  $this->getServer()->getPluginManager()->getPlugin("Grade");
//$this->ru =  $this->getServer()->getPluginManager()->getPlugin("RankUp");
	...
	private function getPlayerNextRank(Player $player){
				$ru = $this->getServer()->getPluginManager()->getPlugin("RankUp");
				
		if($ru !== null){
			$nextrank = $ru->getRankStore()->getNextRank($player);
				
				if($nextrank !== false and !$this->getPlayerPrisonRank($player) == "Z"){
					$namenextrank = $nextrank->getName();
				return $namenextrank;
			}elseif($this->getPlayerPrisonRank($player) == "Z"){
				return "Maximum";
			}
		}
		return "A";
	}
		private function getMoneyNeed(Player $player){
				$ru = $this->getServer()->getPluginManager()->getPlugin("RankUp");
				
		if($ru !== null){
			$nextrank = $ru->getRankStore()->getNextRank($player);
		
            if($this->getPlayerPrisonRank($player) == "Z"){
				return "Maximum";
		}else{
				$moneyneed = $nextrank->getPrice();
			return $moneyneed;
		}
		return "Plugin not found";
	}
	...
	public function process(Player $player, string $string): string{
		/*$string = str_replace("{exp}", $exp, $string);
				$string = str_replace("{maxexp}", $expM, $string);
		 		$string = str_replace("{level}", $grade, $string);
		$exp = $this->grade->getEXP($player->getName());
			$expM = $this->grade->getMaxEXP($player->getName());
			$grade = $this->grade->getGrade($player->getName());*/
		date_default_timezone_set('Asia/Ho_Chi_Minh');
		//$ru = $this->getServer()->getPluginManager()->getPlugin("RankUp");
			
				//$moneyneed = $nextrank->getPrice();
				
		$string = str_replace("{nextrank}", $this->getPlayerNextRank($player), $string);
		$string = str_replace("{moneyneed}", $this->getMoneyNeed($player), $string);
