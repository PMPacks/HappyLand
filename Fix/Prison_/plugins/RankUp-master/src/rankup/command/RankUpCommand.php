                    ...
                    if ($nextRank->getPrice() === 0 || $this->main->isLinkedToEconomy()) {
                        if ($nextRank->getPrice() > 0) {
                            if ($this->main->getEconomy()->take($nextRank->getPrice(), $sender) !== false) {
                                if ($this->main->getPermManager()->addToGroup($sender, $nextRank->getName()) !== false) {//$this->main->getRankUpDoesGroups()->getPlayerGroup($sender)
									$this->main->getServer()->broadcastMessage("§l§b[§a AURA KINGDOM§b ]§6 Người chơi§e ".$sender->getName()."§6 đã nâng cấp lên rank§e ".$nextRank->getName());
